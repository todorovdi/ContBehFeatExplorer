...
if [ $RESAVE_MAT_FILE -ne 0 ]; then
  ipython3 $INTERACTIVE $RUNF -- -r $raw --read_type '""' --to_perform notch
fi
if [ $HIGHPASS -ne 0 ]; then
  ipython3 $INTERACTIVE $RUNF -- -r $raw --read_type notch --to_perform highpass
fi
if [ $tSSS -ne 0 ]; then
  #ipython3 $INTERACTIVE $RUNF -- -r $raw --read_type resample,notch --to_perform tSSS,ICA --badchans_SSS $RESAVE_BAD_CHANS_SSS
  ipython3 $INTERACTIVE $RUNF -- -r $raw --read_type '""' --to_perform tSSS,ICA --badchans_SSS $RESAVE_BAD_CHANS_SSS --output_subdir SSS --recalc_LFPEMG 0
fi
if [ $SSP -ne 0 ]; then
  ipython3 $INTERACTIVE $RUNF -- -r $raw --read_type notch --to_perform SSP,ICA --output_subdir SSP
fi
#ipython3 $INTERACTIVE $RUNF -- -r $raw --read_type resample --to_perform tSSS,ICA

#afterICA will have to be generated by hand

#ipython3 $INTERACTIVE $RUNF -- -r $raw --read_type "" --to_perform resample,notch,highpass
...
if [ $RECONSTRUCT_SOURCES -ne 0 ]; then
  ...
  python3 run_collect_artifacts.py -r $raw --min_dur $MIN_DURATION_QUIET_ALLOWED $s
