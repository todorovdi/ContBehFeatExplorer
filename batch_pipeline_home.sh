if [[ $# -lt 3 ]]; then
  echo "Please put <do_genfeats> <do_PCA> <do_tSNE> <multi_raw_mode>"
  exit 1
fi
...
if [[ $do_genfeats -eq 0 && $do_tSNE -eq 0 && $do_PCA -eq 1 && $allow_multiproc -eq 1 ]]; then
  ...
  #for t in ${raws[@]}; do
    ...
    if [ $COMPL_RUN_JOINT -ne 0 ]; then
      . srun_pipeline_home.sh $do_genfeats $do_PCA $do_tSNE --raw ${raws[$i]} --raw_compl ${raws_compl[$i]}  &
    else
      . srun_pipeline_home.sh $do_genfeats $do_PCA $do_tSNE --raw ${raws[$i]}         &
      . srun_pipeline_home.sh $do_genfeats $do_PCA $do_tSNE --raw ${raws_compl[$i]}   &
    fi
  done
else
  if [ $MULTI_RAW_MODE -eq 1 ]; then
    if [ $MULTI_RAW_STRS_MODE -eq 1 ]; then
      for (( i=0; i<$nraws_strs; i++ )); do
        . srun_pipeline_home.sh $do_genfeats $do_PCA $do_tSNE --raws_multi ${raws_strs[$i]}
      done
    else
      . srun_pipeline_home.sh $do_genfeats $do_PCA $do_tSNE --raws_multi $raws_str
    fi
  else
    for (( i=0; i<=$(( $nraws -1 )); i++ )); do
      #. srun_pipeline_home.sh $do_genfeats $do_PCA $do_tSNE --raw ${raws[$i]} --raw_compl ${raws_compl[$i]}  
